...
* `crewai_runner.run_orchestration(inputs, listing_txt_path=None, photos_folder=None, *, horizon_years=10) -> OrchestrationResult`
  ...
  * **Parity shell by default**: with `AIREAL_LLM_MODE` unset, **no Agent/Task shell is constructed at all** — the actual work delegates to the same deterministic functions the `crew` orchestrator uses. `FinancialForecasterAgent` and `ChiefStrategistAgent` never build a shell, in any mode (neither class has any code path that constructs a `Crew` or calls `kickoff()`); `ListingAnalystAgent` builds its shell lazily and only reaches that construction from `_run_llm()`, which `run()` calls only when `AIREAL_LLM_MODE` is set. Output is byte-identical to the deterministic engine.
...
_Last reconciled: 2026-08-19 against main @ 8ed9397 (post-release CI fix `ee023ee`: corrected the "Parity shell by default" claim — no Agent/Task shell is constructed when `AIREAL_LLM_MODE` is unset; `FinancialForecasterAgent`/`ChiefStrategistAgent` build no shell in any mode, and `ListingAnalystAgent`'s shell is lazy, per `src/orchestrators/crewai_runner.py`'s module docstring). Earlier note: 2026-08-05 against mission/2-wiring-gaps @ 615aaaf (corrected — the previous stamp cited `a626e9d`, a tree that does not contain the per-tag provenance content it claimed to have reconciled: `ObservationProvenance` appears 0 times there and 6 times at HEAD. A provenance stamp that names the wrong tree is the defect this file documents, applied to itself. Guardian M21.) (`CvTaggingOrchestrator.analyze_paths` now also returns `observations` (per-tag provenance)). Earlier note: 2026-08-04 @ d18ee1a (Gate 2 VETO remediation: the CrewAI Orchestrator entry corrected — `crew.kickoff()` **is** called for the Listing Analyst under `AIREAL_LLM_MODE`; the forecast math and verdict stay deterministic in every mode; added the full-suite coverage-gate note)._
