...
* `ListingAnalystAgent` / `FinancialForecasterAgent` / `ChiefStrategistAgent` — the CrewAI wrappers used by `orchestrators/crewai_runner.py`. **Only `ListingAnalystAgent` ever builds a crewai `Agent` shell, and it builds it lazily**: `self._agent` starts `None` and the `agent` property constructs it on first access, reached only from `_run_llm()`. `FinancialForecasterAgent` and `ChiefStrategistAgent` build no shell at all — neither class has any code path that constructs a `Crew` or calls `kickoff()`, in any mode, so there was nothing for an eagerly-built shell to do. (Newer `crewai` (1.x) validates `OPENAI_API_KEY` at `Agent` **construction**, not at call time, so an eager shell in `__init__` made merely instantiating a deterministic agent require a key — see `CHANGELOG.md` [Unreleased].) With `AIREAL_LLM_MODE` unset (the default), all three `run()` methods delegate to the same deterministic functions above for math parity — no shell is built and no `crew.kickoff()` is called. With `AIREAL_LLM_MODE` set **and** a provider key present (`OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `OPENROUTER_API_KEY`), **`ListingAnalystAgent.run()` calls a real `crew.kickoff()`** and an LLM authors the `ListingInsights` it returns (address, amenities, condition tags, defects, notes) — an *observation* layer, falling back to the deterministic `analyze_listing()` if the call or the JSON parse fails. Model-authored tags are stamped `ObservationProvenance(origin="llm", provider=<model name>, provider_kind="model")`; the deterministic fallback keeps its own text/CV provenance and is never relabelled as AI. `FinancialForecasterAgent` and `ChiefStrategistAgent` never call `kickoff()`, in any mode: the forecast math and the BUY/CONDITIONAL/DECLINE verdict are always deterministic. Honors `AIREAL_LLM_MODE` and `AIREAL_DEBUG`.
...
_Last reconciled: 2026-08-19 against main @ 8ed9397 (post-release CI fix `ee023ee`: corrected the "CrewAI components" entry — only `ListingAnalystAgent` ever builds a crewai `Agent` shell, and it does so lazily; `FinancialForecasterAgent`/`ChiefStrategistAgent` build none, per `src/agents/crewai_components.py`'s module docstring). Earlier note: 2026-08-05 against mission/2-wiring-gaps, task 3.2 (the cap-rate-floor rationale line now names the cap and the floor when `market` is supplied, and stays silent when no floor is configured). Earlier note: task 3.1b (deletion half) — removed
