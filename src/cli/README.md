...
| `--ai` | `0` | Passes `use_ai=True` through to `core.cv.build_photo_insights`. The path is wired end-to-end, but the CV providers behind it today are deterministic stubs — not a live model call — so output is unchanged from the default path. The flag exists for when a real vision provider is registered behind that seam. |
