...
class _ValidatesFromDict(Protocol):
    @classmethod
    def model_validate(cls: Any, data: Any) -> Any: ...


T = TypeVar("T", bound=_ValidatesFromDict)


def _maybe_load(model_cls: type[T], arg: str | None) -> T | None:
    if not arg:
        return None
    data = _read_json(Path(arg))

    # pydantic v2: model_validate is the canonical classmethod
    # Return type is the concrete model (T), which satisfies mypy via the Protocol bound.
