...
- Gates cleared: **3 / 5 — ✅ Gate 0, Gate 1, Gate 2 PASSED** · Gate 0 detail: ✅ **PASSED 2026-08-03** (code-reviewer APPROVE · finance-interp PASS ·
...
| 3 | Disposition (wire-first) | 6 | 0 | TODO — 3.1a/3.1b/3.2 chartered + **3.3/3.4/3.5 added 2026-08-05** — M14's fabrication (M22), the contested-branch money leak + laundering (G2-N1/G2-N2, HARD Gate 3 exit), and the `tag_images` promotion | Gate 3 |
...
- **For OPD-4 — rendering the floor is NOT a template edit.** `generate_report`
  (`generator.py:1139`, `write_report` at `:1211` — re-derived 2026-08-05, M23) receives `FinancialForecast` but **not** `FinancialInputs`, and
...
- **Gate 2 (CLI + docs):** ✅ **PASSED 2026-08-05 — guardian NO VETO (approve with modifications),
  code-reviewer APPROVE.** Cleared on the **fourth** attempt; three prior vetoes are recorded below,
  and every blocker they raised was reproduced by the orchestrator before being fixed.

  The guardian attacked R-6 rather than reading it: a warm cache from before provider registration
  (**failed to defeat** — the capability digest is load-bearing; it wrote a second cache directory
  rather than serving the stale entry), a provider registered mid-run (works: 0.86 = 0.7×0.80+0.3
  with both signals in evidence), and a filename token whose label a provider covers (**this one
  landed** — G2-N1, now row 3.4). It also proved `notes` cannot reach the finance core by running a
  notes-only `ListingInsights` through `run_financial_model`: Y1 delta 0.00.

  **Three modifications required before Wave 3 opened, all done:** the misleading safety clause
  corrected in `runner.py` and `core/README.md` (0.30 was cited as the reason a contested claim is
  harmless; it is not — that route reads membership, not confidence), and G2-N1/G2-N2 plus the
  `tag_images` promotion recorded as executable rows 3.4/3.5 rather than prose.

  **Carried as HARD Gate 3 exit criteria:** G2-N1 and G2-N2 (row 3.4), each with a RED-on-revert test.
  **Minor, bound to Gate 3 with the T6 set:** `src/schemas/README.md`'s Usage Example 1 does not run
  (`OperatingExpenses` requires `utilities` and `water_sewer`, which it omits).

- **Gate 2 — history of the three vetoes:** 🔴 **VETOED 2026-08-03 by principles-guardian.** code-reviewer APPROVE (no
